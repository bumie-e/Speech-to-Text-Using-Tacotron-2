...
def norm_data(args):
	...
	supported_datasets = ['LJSpeech-1.0', 'LJSpeech-1.1', 'M-AILABS', 'SwissAudio']
	if args.dataset not in supported_datasets:
		raise ValueError('dataset value entered {} does not belong to supported datasets: {}'.format(
			args.dataset, supported_datasets))

	if args.dataset.startswith('SwissAudio'):
...
def main():
	...
	parser.add_argument('--dataset', default='SwissAudio')
	parser.add_argument('--language', default='en_US')
	parser.add_argument('--voice', default='male')
	parser.add_argument('--reader', default='john_wyss')
	parser.add_argument('--merge_books', default='False')
	parser.add_argument('--book', default='johnwyss')
